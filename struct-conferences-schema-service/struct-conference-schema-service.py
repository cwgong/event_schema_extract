...
import sys
sys.path.append('./conferences_Extract')
...
class PredictHandler(tornado.web.RequestHandler):      
    ...
    def post(self):
        ...
        try:
            ...
            abstract = dic_of_param.get('abstract')
            title = dic_of_param.get('title')
            publishAt = dic_of_param.get('publishAt')
            # logging.info("\nlist_of_doc size: ---------> " + str(len(list_of_doc)))
            
            self.handler_predict(abstract,title,publishAt)
    ...
    def handler_predict(self, abstract,title,publishAt):

        schema_all_article = etld.extract_schema.get_conference_schema(abstract,title,publishAt)
        # result = []
        ...
        schema_all_article = json.dumps(schema_all_article, ensure_ascii=False)
        self.write(schema_all_article)
